...
router.post('/notifications', JwtUtil.checkToken, async function (req, res) {
  const name = req.body.name;
  const notification = { name: name };
  const result = await NotificationDAO.insert(notification);
  res.json(result);
});
// NotificationDAO
...
router.post('/products', JwtUtil.checkToken, async function (req, res) {
  const name = req.body.name;
  const price = req.body.price;
  const cid = req.body.notification;
  const image = req.body.image;
  const now = new Date().getTime(); // milliseconds
  const notification = await NotificationDAO.selectByID(cid);
  const product = { name: name, price: price, image: image, cdate: now, notification: notification };
  const result = await ProductDAO.insert(product);
  res.json(result);
});
...
router.put('/products/:id', JwtUtil.checkToken, async function (req, res) {
  ...
  const now = new Date().getTime(); // milliseconds
  const notification = await NotificationDAO.selectByID(cid);
  const product = { _id: _id, name: name, price: price, image: image, cdate: now, notification: notification };
  const result = await ProductDAO.update(product);
  res.json(result);
});
